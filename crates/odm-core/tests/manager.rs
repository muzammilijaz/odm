...
async fn end_to_end_download_lands_in_the_right_category() {
    ...
    let task = manager
        .add_download(&format!("{base}/file/movie.mkv"), None, false, None)
        .await
        .unwrap();
...
async fn unrecognized_extension_goes_to_general() {
    ...
    let task = manager
        .add_download(&format!("{base}/file/data.xyz123"), None, false, None)
        .await
        .unwrap();
...
async fn pause_then_resume_still_completes() {
    ...
    let task = manager
        .add_download(&format!("{base}/file/archive.zip"), None, false, None)
        .await
        .unwrap();
...
async fn concurrency_limit_is_respected() {
    ...
    let t1 = manager
        .add_download(&format!("{base}/file/a.zip"), None, false, None)
        .await
        .unwrap();
    let t2 = manager
        .add_download(&format!("{base}/file/b.zip"), None, false, None)
        .await
        .unwrap();
...
async fn repeated_direct_downloads_get_unique_filenames() {
    ...
    let dir = tempdir().unwrap();
    let manager = TaskManager::new(db, DownloadConfig::default(), dir.path(), 2);
    let url = format!("{base}/file/muzammil.exe");

    let first = manager.add_download(&url, None, false, None).await.unwrap();
    wait_for_status(
        &manager,
        first.id,
        TaskStatus::Completed,
    ...
    .await;
    let second = manager.add_download(&url, None, false, None).await.unwrap();
    wait_for_status(
        &manager,
        second.id,
        TaskStatus::Completed,
