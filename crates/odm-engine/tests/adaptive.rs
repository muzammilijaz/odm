...
async fn spawn_hls_server() -> String {
    ...
    let app = Router::new()
        ...
        .route(
            "/seg0.ts",
            get(|| async { (StatusCode::OK, [(header::CONTENT_TYPE, "video/mp2t")], vec![0u8; 188]) }),
        )
        .route(
            "/seg1.ts",
            get(|| async { (StatusCode::OK, [(header::CONTENT_TYPE, "video/mp2t")], vec![1u8; 188]) }),
        );
...
async fn detects_hls_by_extension() {
    ...
    let kind = detect_stream_kind(&client, "https://example.com/video/stream.m3u8").await.unwrap();
...
async fn detects_dash_by_extension() {
    let client = reqwest::Client::new();
    let kind = detect_stream_kind(&client, "https://example.com/video/manifest.mpd").await.unwrap();
...
async fn downloads_and_remuxes_a_simple_hls_stream() {
    ...
    let path = download_adaptive(&client, &format!("{base}/stream.m3u8"), &dest, StreamKind::Hls).await.unwrap();
